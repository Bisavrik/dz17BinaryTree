...
void BinaryTree::displayTranslation(const char* word)
{
    ...
    {
        node->access_count++;
        cout << "Перевод слова \"" << node->word << "\": " << node->translation << endl;
    }
    else
    {
...
void BinaryTree::replaceTranslation(const char* word, const char* new_translation)
{
    ...
    if (node) 
    {
        ...
        cout << "Перевод слова \"" << word << "\" заменён на \"" << new_translation << "\"." << endl;
    }
    else
    {
        cout << "Слово \"" << word << "\" не найдено в словаре." << endl;
...
void BinaryTree::displayMostPopularWord()
{
    ...
    if (most_popular) 
    {
        cout << "Самое популярное слово: \"" << most_popular->word << "\" с переводом \"" << most_popular->translation << "\" (обращений: " << most_popular->access_count << ")" << endl;
    }
}
...
void BinaryTree::displayLeastPopularWord() 
{
    ...
    if (least_popular)
    {
        cout << "Самое непопулярное слово: \"" << least_popular->word << "\" с переводом \"" << least_popular->translation << "\" (обращений: " << least_popular->access_count << ")" << endl;
    }
}